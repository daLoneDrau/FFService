...
public class FFHexagonController {
    ...
    public List<Resource<FFHexagonEntity>> getByHorizontalDistance(
        ...
        Iterator<FFHexagonEntity> iter =
                repository.findByHorizontalDistance(horizontalDistance)
                        .iterator();
        List<Resource<FFHexagonEntity>> resources =
                new ArrayList<Resource<FFHexagonEntity>>();
        while (iter.hasNext()) {
    ...
    public List<Resource<FFHexagonEntity>> getByVerticalDistance(
        ...
        Iterator<FFHexagonEntity> iter =
                repository.findByVerticalDistance(verticalDistance)
                        .iterator();
        List<Resource<FFHexagonEntity>> resources =
                new ArrayList<Resource<FFHexagonEntity>>();
        while (iter.hasNext()) {
    ...
    public List<Resource<FFHexagonEntity>> save(
            @RequestBody
            final List<FFHexagonEntity> entities) {
        List<Resource<FFHexagonEntity>> resources =
                new ArrayList<Resource<FFHexagonEntity>>();
    ...
    private void setIdFromRepository(final FFHexagonEntity entity) {
        ...
        try {
            ...
            try {
                ...
                System.out.println(
                        "Cannot get Entity FFHexagonEntity from Repository by name");
            }
            if (method != null
                    && field != null) {
            ...
            if (old == null
                ...
                try {
                    ...
                    // TODO Auto-generated catch block
                    System.out.println(
                            "Cannot get Entity FFHexagonEntity from Repository by code");
                }
                if (method != null
                        && field != null) {
                    field.setAccessible(true);
            ...
            System.out.println(
                    "Cannot get Entity FFHexagonEntity from Repository by name or code");
        }
        if (old != null
                && old.size() == 1) {
